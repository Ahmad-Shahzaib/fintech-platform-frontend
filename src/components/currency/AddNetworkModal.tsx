...
const AddNetworkModal: React.FC<AddNetworkModalProps> = ({ isOpen, onClose, onSubmit, initialData = null, mode = 'create', isLoading = false }) => {
      ...
      <div className="fixed inset-0  flex items-center justify-center z-50">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-3xl p-6">
          <div className="flex items-center justify-center py-8">
            <svg className="animate-spin h-6 w-6 text-gray-600 dark:text-gray-300 mr-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
            </svg>
            <span className="text-gray-700 dark:text-gray-300">Loading network...</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0  flex items-center justify-center z-[100000] bg-black/60">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-3xl p-6 max-h-[80vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white">{isView ? 'View Network' : isEdit ? 'Edit Network' : 'Add New Network'}</h2>
