...
const ManageFaqs: React.FC = () => {
  ...
  useEffect(() => {
    if (success) {
      setQuestion('');
      setAnswer('');
      // show success alert when a FAQ is created
      try {
        showAlert('FAQ added', 'success');
      } catch (e) {
        // ignore if alert context not available in tests/SSR
      }
  ...
  // Track delete lifecycle: when deletingId goes from a value to falsy, deletion finished
  useEffect(() => {
    const prev = prevDeletingId.current;
    const now = deletingId ? String(deletingId) : null;
    if (prev && !now) {
    ...
    prevDeletingId.current = now;
  }, [deletingId, deleteError, dispatch, showAlert]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 rounded-lg shadow-md bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Manage FAQs</h3>
        <Button
      ...
      <div className="space-y-4">
        {fetching ? (
          <p className="text-gray-700 dark:text-gray-300">Loading FAQs...</p>
        ) : (
          (faqs || []).map((f: any, idx: number) => (
            <div key={f.id || idx} className="bg-white dark:bg-gray-800 shadow dark:shadow-gray-700 rounded-lg p-4">
              <div className="flex items-start justify-between">
                <div>
                  <h4 className="font-semibold text-gray-800 dark:text-gray-100">{f.question}</h4>
                  <p className="text-gray-600 dark:text-gray-300 mt-2">{f.answer}</p>
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center z-[100000]">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-gray-700 z-10 w-full max-w-xl p-6 text-gray-900 dark:text-gray-100">
            ...
            <form onSubmit={(e) => { handleSubmit(e); }} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Answer</label>
                <textarea
                  value={answer}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 flex items-center justify-center z-[100000]">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-gray-700 z-10 w-full max-w-md p-6 text-gray-900 dark:text-gray-100">
            <h4 className="text-lg font-semibold mb-4">Confirm Delete</h4>
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-4">Are you sure you want to delete this FAQ?</p>
            {deleteTarget?.question && <p className="text-gray-600 dark:text-gray-300 italic mb-4">"{deleteTarget.question}"</p>}
